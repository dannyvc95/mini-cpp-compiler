...
class lista{
    public:
        ...
        char buscarTPAR(string, string, int);
        T recupera(nodo<T>* p);
        void imprimir();
        void borrartodo();
    private:
...
void lista<T>::insertarError(int idE, T dato){
	...
	switch(idE){
        ...
        break;
        case 5:
			cout << "\tERROR Re Declaracion: \""<< dato << "\" Esta funcion no se ha declarado anteriormente" << endl;
			errorStr = "\tERROR Re Declaracion: \"" + dato + "\" Esta funcion no se ha declarado anteriormente";
		break;
		case 6:
			cout << "\tERROR Return: "<< dato << " El Valor de retorno no es del mismo tipo" << endl;
			errorStr = "\tERROR Return: " + dato + " El Valor Regresa no es del mismo tipo";
		break;
		case 7:
			cout << "\tERROR Argumentos: \""<< dato << "\" No hay suficientes argumentos" << endl;
			errorStr = "\tERROR Argumentos: \"" + dato + "\" No hay suficientes argumentos";
		break;
		case 8:
			cout << "\tERROR Tipo Argumentos: \""<< dato << "\" La  funcion no concuerda en argumentos con la declaracion" << endl;
			errorStr = "\tERROR Tipo Argumentos: \"" + dato + "\" La  funcion no concuerda en argumentos con la declaracion";
		break;
	}
...
template <class T>
char lista<T>::buscarTPAR(string id, string ambito, int campo){
    nodo<T>* temporal = encabezado;
    while(temporal != NULL){
        if(temporal->getdato().getID() == id and temporal->getdato().getAMB() == ambito){
            return temporal->getdato().getVectorPAram(campo);
        }
        temporal = temporal->getsiguiente();
    }
}
